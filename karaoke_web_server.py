...
class TaskManager:
    ...
    def run_command_async(self, project_name, cmd, task_type, total_duration=None):
        def worker():
            print(
                f"[RUN] Iniciando comando para {project_name} ({task_type}): {' '.join(cmd)}"
            )
            self.set_task(
                project_name,
                task_type,
            ...
            try:
                ...
                if process.returncode == 0:
                    self.set_task(
                        ...
                        status="success",
                        progress=100,
                        logs=self.tasks[project_name]["logs"]
...
def parse_multipart_data(rfile, headers):
    ...
    for part in parts:
        ...
        if name:
            if filename:
                files[name] = {"filename": filename, "content": part_content}
            else:
...
class KaraokeHTTPHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/api/projects":
                ...
                if (
                    ...
                    if cfg_file.exists():
                        try:
                            ...
                            projects.append(
                                {"name": item.name, "title": item.name, "artist": ""}
                            )
            ...
            info = {
                ...
                "status": {
                    ...
                    and (project_path / "instrumental.mp3").stat().st_size > 0,
                    ...
                    "video_exists": (
                        out_dir
                        / get_dynamic_output_name(config, project_name, preview=False)
                    ).exists(),
                    "preview_exists": (
                        out_dir
                        / get_dynamic_output_name(config, project_name, preview=True)
                    ).exists(),
                },
                "images": images_list,
            }
            ...
            def scan_videos(directory, parent_name):
                ...
                for item in sorted(directory.iterdir()):
                    if item.is_dir():
                        collection_name = (
                            item.name.replace("_", " ").replace("-", " ").title()
                        )
                        for sub_item in sorted(item.iterdir()):
                            if (
                                sub_item.is_file()
                                and sub_item.suffix.lower() in video_exts
                            ):
                        ...
                        collection_name = (
                            "General" if parent_name == "base_videos" else "Otros"
                        )
                        collections.add(collection_name)
                        videos.append(
                            {
        ...
        # Endpoint para listar las imágenes pre-cargadas de base_images/
            ...
            return self.send_json(
                {"images": images, "collections": sorted(collections)}
            )
        ...
        # Endpoint para servir archivos de imagen de base_images/ (para miniaturas)
            ...
            if not map_file.exists():
                return self.send_error_json(
                    "El mapa no ha sido generado aún. Corre la transcripción primero."
                )
        ...
        # Endpoint para listar fuentes disponibles del sistema
            ...
            for p in FONT_PATHS:
                if Path(p).exists():
                    available.append({"name": Path(p).stem, "path": p})
            ...
            for ttf in PROJECTS_DIR.glob("**/*.ttf"):
                if ".venv" not in ttf.parts:
                    available.append(
                        {"name": f"[Proyecto] {ttf.name}", "path": str(ttf.resolve())}
                    )
        ...
        # Endpoint para servir el archivo de fuente binario (.ttf/.otf) al cliente
            ...
            if (
                not filepath.exists()
                or filepath.is_dir()
                or filepath.suffix.lower() not in {".ttf", ".otf"}
            ):
                return self.send_error_json(
                    "Archivo de fuente no válido o no encontrado", 404
                )
    ...
    def do_POST(self):
        ...
        if path == "/api/init":
            ...
            if not name:
                return self.send_error_json(
                    "El nombre del proyecto contiene caracteres no válidos"
                )
            ...
            cfg["output_name"] = get_dynamic_output_name(
                cfg, name, preview=False, mode=cfg["mode"]
            )
        ...
        # ── CONFIGURACIÓN (JSON) ──────────────────────────────────────────────
            ...
            try:
                ...
                new_config["output_name"] = get_dynamic_output_name(
                    new_config, project_name, preview=False
                )
                cfg_file.write_text(
        ...
        # ── SUBIR IMAGEN (MULTIPART) ──────────────────────────────────────────
            ...
            if ext not in {".png", ".jpg", ".jpeg", ".webp"}:
                return self.send_error_json(
                    "Formato de imagen no soportado (debe ser PNG, JPG, JPEG o WEBP)"
                )

            # Limpiar nombre
            clean_name = Path(filename).stem
            clean_name = (
                "".join(c for c in clean_name if c.isalnum() or c in ("_", "-")) + ext
            )
        ...
        # ── SUBIR VIDEO (MULTIPART) ───────────────────────────────────────────
            ...
            if ext not in {".mp4", ".mov", ".avi", ".mkv", ".webm"}:
                return self.send_error_json(
                    "Formato de video no soportado (debe ser MP4, MOV, AVI, MKV o WEBM)"
                )
            ...
            return self.send_json(
                {"success": True, "filename": f"loop_videos/{clean_name}"}
            )
        ...
        # ── SUBIR MARCA DE AGUA (MULTIPART) ───────────────────────────────────
            ...
            if ext not in {".png", ".jpg", ".jpeg", ".webp"}:
                return self.send_error_json(
                    "Formato de marca de agua no soportado (debe ser PNG, JPG, JPEG o WEBP)"
                )
            ...
            if cfg_file.exists():
                try:
                    ...
                    print(
                        f"[WARNING] No se pudo guardar config.json tras subir marca de agua: {e}"
                    )
        ...
        # ── IMPORTAR SRT ──────────────────────────────────────────────────────
            ...
            if not project_name or not srt_text:
                return self.send_error_json(
                    "Parámetros incorrectos (falta project o srt_text)"
                )
            ...
            entries = []
            for m in pat.finditer(srt_text):
                text = m.group(3).strip().replace("\n", " ")
                if text:
                    entries.append(
                        {
                            "start": round(ts2sec(m.group(1)), 3),
                            "end": round(ts2sec(m.group(2)), 3),
                            "text": text,
                        }
                    )
            ...
            if not entries:
                return self.send_error_json(
                    "No se encontraron subtítulos válidos en el archivo SRT"
                )

            segments = []
            words_flat = []
            for idx, entry in enumerate(entries):
                words_text = entry["text"].split()
                duration = entry["end"] - entry["start"]
                word_dur = duration / max(len(words_text), 1)

                words = []
                for wi, wt in enumerate(words_text):
                    w_start = round(entry["start"] + wi * word_dur, 3)
                    w_end = round(entry["start"] + (wi + 1) * word_dur, 3)
                    words.append(
                        {
                            "word": wt,
                            "start": w_start,
                    ...
                    words_flat.append(
                        {
                            ...
                            "start": w_start,
                            "end": w_end,
                            "duration_ms": int(round((w_end - w_start) * 1000)),
                            "confidence": 1.0,
                            "segment_id": idx,
        ...
        # ── EJECUTAR SEPARADOR ────────────────────────────────────────────────
            ...
            if not audio_path.exists() or audio_path.stat().st_size == 0:
                return self.send_error_json(
                    "El archivo de audio no existe o está vacío. Por favor sube un archivo de audio válido en el Paso 1."
                )
        ...
        # ── EJECUTAR WHISPER ──────────────────────────────────────────────────
            ...
            try:
                ...
                print(
                    f"[WARNING] No se pudo guardar config.json al correr whisper: {e}"
                )
            ...
            if use_vocals:
                ...
                if not vocals_path.exists() or vocals_path.stat().st_size == 0:
                    return self.send_error_json(
                        "No se encontró el archivo de voz limpia (vocals.mp3) o está vacío. Por favor realiza la separación de audio primero."
                    )
                    ...
                    if status["status"] == "success":
                        ...
                        if not map_json.exists():
                            ...
                            if temp_map.exists():
                                ...
                                task_manager.append_log(
                                    project_name,
                                    "✅ Mapa map.json generado exitosamente en output/map.json.\n",
                                )
                        break
        ...
        # ── EJECUTAR ASSEMBLER (RÉNDER) ───────────────────────────────────────
            ...
            watermark_preset = data.get(
                "watermark_preset", cfg.get("watermark_preset", "none")
            )
            watermark_pos = data.get(
                "watermark_pos", cfg.get("watermark_pos", "top-right")
            )
            ...
            cfg["output_name"] = get_dynamic_output_name(
                cfg, project_name, preview=False, mode=mode
            )
            ...
            if watermark_preset == "custom":
                ...
                preset_path = (
                    WORKSPACE_DIR / "base_images" / f"preset_{watermark_preset}.png"
                )
                if not preset_path.exists():
                    try:
                        ...
                        print(
                            f"[WARNING] No se pudo generar preset {watermark_preset}: {e}"
                        )
            ...
            out_name = get_dynamic_output_name(
                cfg, project_name, preview=preview, mode=mode
            )
            ...
            if watermark_path:
                cmd += [
                    ...
                    str(watermark_path),
                    "--watermark-pos",
                    cfg.get("watermark_pos", "top-right"),
                    "--watermark-dur",
                    cfg.get("watermark_dur", "intro"),
                ]
...
def run_server(port=8080):
    ...
    if not WEB_UI_DIR.exists():
        print(
            f"[WARNING] Carpeta {WEB_UI_DIR} no encontrada. Asegurate de crear los archivos del frontend."
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Inicia el servidor web local para Karaoke Maker"
    )
    parser.add_argument(
